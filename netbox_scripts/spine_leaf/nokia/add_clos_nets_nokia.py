...
def main():
    ...
    for spine_name in input_data['devices']['spine']:
        ...
        print(f"\n{spine_name.upper()}")
        add_loopback_interface(nb_spine, 'system0', 0, [nb_subnets['underlay_loopbacks']])
        add_loopback_interface(nb_spine, 'lo50', 2, [nb_subnets['overlay_loopback4'], nb_subnets['overlay_loopback6']])
    ...
    for leaf_name in input_data['devices']['leaf']:
        ...
        for index, spine in enumerate(spines):
            add_spine_link(spine, leaf, spine_port, 56-index)
        ...
        # For test lab
        vlan_prefix = f"private{leaf_name.split('-')[0].lstrip('lsw')}"
        add_leaf_vlan(vlan_prefix, leaf)

        # Leaf underlay loopback
        add_loopback_interface(leaf, 'system0', 0, [nb_subnets['underlay_loopbacks']])
        # Leaf overlay loopback
        add_loopback_interface(leaf, 'lo50', 2, [nb_subnets['overlay_loopback4'], nb_subnets['overlay_loopback6']])
...
def add_loopback_interface(nb_device, int_name, vrf_id, parent_subnets):
    ...
    if loop_int.count_ipaddresses > 0:
        print(f"    Interface {loop_int} already has IP addresses... skipping.")
        return
...
def add_spine_link(spine, leaf, spine_port, leaf_port):
    ...
    leaf_int = get_nb_int(leaf, f"ethernet-1/{leaf_port}")
    leaf_int.mtu = 9192
    leaf_int.save()
    spine_int = get_nb_int(spine, f"ethernet-1/{spine_port}")
    spine_int.mtu = 9192
    spine_int.save

    # Add connection between ports if needed
    if not leaf_int.connected_endpoints and not spine_int.connected_endpoints:
        print(f"    Adding link from {leaf.name} ethernet-1/{leaf_port} to {spine.name} ethernet-1/{spine_port}...", end="")
        new_cable = nb.dcim.cables.create(color="ffeb3b", type="smf", status='connected', label=f"dummy{leaf_int.id}{spine_int.id}",
                a_terminations=[{"object_type": "dcim.interface", "object_id": spine_int.id}],
                b_terminations=[{"object_type": "dcim.interface", "object_id": leaf_int.id}])
        print(" done.")
    elif leaf_int.connected_endpoints and spine_int.connected_endpoints:
        # Quit if they are not connected to each other
        if leaf_int.connected_endpoints[0].id != spine_int.id:
            print(f"    ERROR: {leaf.name} ethernet-1/{leaf_port} connection does not go to {spine.name} ethernet-1/{spine_port}.  Quitting!")
            sys.exit(1)
    else:
...
def add_leaf_vlan(vlan_type, leaf):
    ...
    irb_parent = nb.dcim.interfaces.get(device_id=leaf.id, name="irb0")
    print(f"      Creating interface irb.{vlan_id} on {leaf.name}...", end="")
    irb_int = nb.dcim.interfaces.create(device=leaf.id, name=f"irb0.{vlan_id}", 
        type='virtual', vrf=2, description=vlan_name, parent=irb_parent.id, mode="access",
        untagged_vlan=new_vlan.id)
    print(" done.")
    ...
    for prefix in [v6_subnet, v4_subnet]:
        ...
        new_ip.dns_name=f"irb0-{vlan_id}.{leaf.name}.{site.slug}.wmnet"
        print(" done.")
        new_ip.save()
